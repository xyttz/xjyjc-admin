...
import { getToken, removeToken, setToken, setRefreshToken, removeRefreshToken } from '@/utils/token'
...
const actions = {
  ...
  async login({ commit }, userInfo) {
    ...
    if (access_token) {
      ...
      const err = `登录接口异常，未正确返回${access_token}...`
  ...
  async getUserInfo({ commit, dispatch, state }) {
    ...
    /*
     const {
      data: { roles, nickname, avatar, perms },
    } = await getUserInfo()
    if ((nickname && !isString(nickname)) || (avatar && !isString(avatar)) || (roles && !isArray(roles)) || (perms && !isArray(perms))) {
