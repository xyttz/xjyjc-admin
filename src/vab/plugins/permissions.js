...
import { routesWhiteList, authentication } from '@/config'
...
router.beforeEach(async (to, from, next) => {
  ...
  // console.log('user/token:' + hasToken + ' |topath:' + to.path + ' |routes.length:' + store.getters['routes/routes'].length)
  // 禁止已登录用户返回登录页
  if (hasToken) {
    if (to.path === '/login') {
      next({ path: '/' })
      ...
      const hasRoles = store.getters['acl/role'] && store.getters['acl/role'].length > 0
