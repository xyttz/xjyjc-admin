...
export function filterRoutes(routes, rolesControl, baseUrl = '/') {
  // routes.map((r) => {
  //   r = { ...r }
  //   console.log('route.path:' + r.path + ' /acl/role:' + store.getters['acl/role'] + ' /guard:' + JSON.stringify(r.meta.guard))
  // })
  return routes
    .filter((route) => (rolesControl && route.meta && route.meta.guard ? hasRolePermission(route.meta.guard) : true))
    .map((route) => {
      route = { ...route }
...
export function handleMatched(routes, name) {
  return routes.filter((route) => route.childrenNameList.indexOf(name) + 1).flatMap((route) => (route.children ? [route, ...handleMatched(route.children, name)] : [route]))
}
...
export function handleTabs(tag, init = false) {
  let parentIcon = null
  if (tag.matched) for (let i = tag.matched.length - 2; i >= 0; i--) if (!parentIcon && tag.matched[i].meta.icon) parentIcon = tag.matched[i].meta.icon
