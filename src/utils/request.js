...
const requestConf = (config) => {
  ...
  // console.log('request config token:' + token + ' |config.url' + config.url)
...
const tryRefreshToken = async (config) => {
  ...
  */
  if (refreshToking == false) {
    refreshToking = true // 切换正在刷新标识，其他请求先进请求等待队列中
    ...
    return store
      ...
      .then((token) => {
        ...
        requests.forEach((callback) => callback(token)) // 已成功刷新token，队列中的所有请求重试
        requests = []
        return instance(requestConf(config))
      })
      .catch(() => {
...
const handleData = async ({ config, data, status = 0, statusText }) => {
  ...
  switch (code) {
    ...
    case 401:
      ...
      break
    case 402:
      console.log('token to exceed the time limit.......')
