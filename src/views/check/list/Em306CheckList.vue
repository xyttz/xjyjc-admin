<template>
  <div v-loading="downloadLoading" class="components-container">
    ...
    <el-table ref="table" v-loading="loading" border :data="pageList" default-expand-all row-key="entkey" :tree-props="{ children: 'children', hasChildren: 'hasChildren' }">
      ...
      <el-table-column align="center" fixed="right" label="操作" width="415">
        <template slot-scope="scope">
          <el-button
            v-if="scope.row.dtype == 'CHILDREN'"
            v-permissions="['chk:link:query']"
            icon="el-icon-search"
            plain
            size="mini"
            type="primary"
            @click.stop="handleCheckDetail(scope.row)"
          >
            查看检测单
          </el-button>
          <el-button
            v-if="scope.row.dtype == 'PARENT'"
            v-permissions="['chk:link:pkpmsync']"
            icon="el-icon-share"
            plain
            size="mini"
            type="primary"
          ...
          <el-button
            ...
            size="mini"
            type="primary"
            @click.stop="handleOrgReport(scope.row)"
          >
            查看原始记录
          </el-button>
        </template>
  ...
  export default {
    ...
    methods: {
      handleQuery() {
        ...
        list(this.queryParams, this.pathType).then((response) => {
      ...
      handleSync(row) {
        ...
        console.log(JSON.stringify(row))
        // const entkey = row.entkey
        let entnum = row.entnum
        syncCheckData(entnum, this.pathType).then(
          (res) => {
            Loading.close()
