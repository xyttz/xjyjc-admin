  ...
  <div class="perm-container">
    <el-card class="box-card" shadow="always">
      ...
      <el-table ref="table" v-loading="loading" border :data="pageList" :max-height="tableHeight" size="mini">
        ...
        <el-table-column fixed="left" header-align="center" label="检测名称" prop="checkname" width="250" />
        <!-- <el-table-column align="center" label="状态" width="70">
          <template slot-scope="scope">
            <el-switch v-model="scope.row.status" :active-value="1" :inactive-value="0" @change="handleStatusChange(scope.row)" />
          </template>
        </el-table-column> -->
        <el-table-column header-align="center" label="检测说明" prop="checkexp" />
        <el-table-column align="center" label="创建时间" prop="gmtCreate" width="100" />
        <el-table-column align="center" label="更新时间" prop="gmtModified" width="100" />
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleEdit(row) {
        console.log(row.entrustsPath)

        if (row.entrustsPath) {
          this.$router.push({
            // path: '/entrust/components/samples/E301',
            path: row.entrustsPath,
          ...
          this.$baseMessage('目前不支持此项目的委托复核，功能迭代中...', 'info')
        }
      },
    },
  }
