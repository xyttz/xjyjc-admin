<template>
  <div v-loading="listLoading" class="app-container">
    <table style="width: 100%; border-collapse: collapse">
      <tr>
        <td colspan="8" style="background-color: #5e5c5f; color: #fff">基础信息-环刀法测定土的密度（国标）</td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td colspan="6">
          <el-checkbox v-model="form.checkitem1">土的密度试验(环刀法)</el-checkbox>
        </td>
      </tr>
      <tr>
      ...
      <tr>
        <td colspan="8">
          <el-form ref="form" :inline="true" label-width="150px" :model="form" :rules="rules" size="small">
            ...
            <el-form-item label="碾压厚度" prop="cnyhd">
              <el-input v-model="form.cNyhd" style="width: 200px" />
            </el-form-item>
            <el-form-item label="击实试验编号" prop="csybh">
              <el-input v-model="form.cSybh" style="width: 200px" />
            </el-form-item>
            <el-form-item label="取样日期" prop="recdate">
              <el-date-picker
            ...
            <el-form-item label="工程地址" prop="ctfdz">
              <el-autocomplete
                v-model="form.ctfdz"
            ...
            <el-form-item label="取样数量" prop="cpcount">
              <el-input-number v-model="form.cPcount" controls-position="right" :min="1" style="width: 200px" />
            </el-form-item>
            <el-form-item label="土壤类别" prop="ctrlb">
              <el-select v-model="form.cTrlb" placeholder="请选择土壤类别" style="width: 200px">
                <el-option v-for="item in cTrlbOptions" :key="item.name" :label="item.name" :value="item.value" />
              </el-select>
            </el-form-item>
            <el-form-item label="土壤描述" prop="cms">
              <el-input v-model="form.cMs" style="width: 200px" />
            </el-form-item>
            <el-form-item label="击实方法" prop="cjsf">
              <el-select v-model="form.cJsf" placeholder="请选择击实方法" style="width: 200px">
                <el-option v-for="item in cJsfOptions" :key="item.name" :label="item.name" :value="item.value" />
              </el-select>
            </el-form-item>
            ...
            <el-form-item label="碾压机械" prop="cjx">
              <el-input v-model="form.cJx" style="width: 200px" />
            </el-form-item>
            <el-form-item label="碾压变数" prop="cbs">
              <el-input v-model="form.cBs" style="width: 200px" />
            </el-form-item>
            <el-form-item label="取土标高" prop="ctbg">
              <el-input v-model="form.cTbg" style="width: 200px" />
            </el-form-item>
            <el-form-item label="取土深度" prop="csd">
              <el-input v-model="form.cSd" style="width: 200px" />
            </el-form-item>
            <el-form-item label="最佳含水率(%)" prop="chsl">
              <el-input v-model="form.cHsl" style="width: 200px" />
            </el-form-item>
            <el-form-item label="最大干密度(g/cm³)" prop="cmgmd">
              <el-input v-model="form.cMgmd" style="width: 200px" />
            </el-form-item>
            <el-form-item label="设计压实度(%)" prop="cysd">
              <el-input v-model="form.cYsd" style="width: 200px" />
            </el-form-item>
            <el-form-item label="要求试验日期" prop="repcondate">
              <el-date-picker
                v-model="form.cQdate"
                :editable="false"
                :picker-options="pickerOptions"
                placeholder="选择要求试验日期"
                style="width: 200px"
                type="date"
                value-format="yyyy-MM-dd"
              />
            </el-form-item>
            <el-form-item label="最小干密度(g/cm³)" prop="cngmd">
              <el-input v-model="form.cNgmd" style="width: 200px" />
            </el-form-item>
            <el-form-item label="要求压实系数" prop="cysxs">
              <el-input v-model="form.cYsxs" style="width: 200px" />
            </el-form-item>
...
<script>
  ...
  import * as EM324CONSTS from '@/consts/EM324_CONSTS'
  ...
  export default {
    ...
    methods: {
      ...
      loadData() {
        ...
        if (this.id) {
          getByEntId(this.id, '324').then((response) => {
        ...
        this.$refs.form.validate((formValid) => {
          if (formValid) {
            ...
            if (this.id) {
              ...
              updateEntrust(this.id, this.form, '324').then(
                (res) => {
                  getBySampleKey(this.form.sampnum, '324').then((response) => {
                    this.form = response.data
      ...
      async loadHisColunmDatas() {
        this.listLoading = true
        const { data } = await hislist(EM324CONSTS.ELMT_TYPE, EM324CONSTS.ELMT_CODE_STRUCTPARTS)
        this.structparts = data //结构部位
        await hislist(EM324CONSTS.ELMT_TYPE, EM324CONSTS.ELMT_CODE_CHECKTYPES).then((response) => {
          this.checktypes = response.data //检验类别
        })
        await hislist(EM324CONSTS.ELMT_TYPE, EM324CONSTS.ELMT_CODE_PIHAOS).then((response) => {
          this.pihaos = response.data //炉、批号
        })
        await hislist(EM324CONSTS.ELMT_TYPE, EM324CONSTS.ELMT_CODE_UNITNUMS).then((response) => {
          this.unitnums = response.data //生产厂家
        })
        await hislist(EM324CONSTS.ELMT_TYPE, EM324CONSTS.ELMT_CODE_STANDNAMES).then((response) => {
          this.standnames = response.data //检验标准
        })
        await hislist(EM324CONSTS.ELMT_TYPE, EM324CONSTS.ELMT_CODE_EXPLS).then((response) => {
          this.expls = response.data //备注
        })
        await hislist(EM324CONSTS.ELMT_TYPE, EM324CONSTS.ELMT_CODE_BKCOL1S).then((response) => {
          this.bkcol1s = response.data //母材单号
        })
        await hislist(EM324CONSTS.ELMT_TYPE, EM324CONSTS.ELMT_CODE_BKCOL2S).then((response) => {
          this.bkcol2s = response.data //技术等级
        })
        await hislist(EM324CONSTS.ELMT_TYPE, EM324CONSTS.ELMT_CODE_BKCOL3S).then((response) => {
          this.bkcol3s = response.data //焊条型号
        })
        await hislist(EM324CONSTS.ELMT_TYPE, EM324CONSTS.ELMT_CODE_RECADDS).then((response) => {
          this.recadds = response.data //取样地点
        })
        await hislist(EM324CONSTS.ELMT_TYPE, EM324CONSTS.ELMT_CODE_PAIHAOS).then((response) => {
          this.paihaos = response.data //牌号
        })
        await hislist(EM324CONSTS.ELMT_TYPE, EM324CONSTS.ELMT_CODE_SAMPNUMS).then((response) => {
