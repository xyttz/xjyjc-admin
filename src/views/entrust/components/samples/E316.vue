<template>
  <div v-loading="listLoading" class="app-container">
    <table style="width: 100%; border-collapse: collapse">
      <tr>
        <td colspan="8" style="background-color: #5e5c5f; color: #fff">基础信息-烧结多孔砖</td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td colspan="6">
          ...
          <el-checkbox v-model="form.checkitem2">尺寸偏差</el-checkbox>
        </td>
      </tr>
      <tr>
...
<script>
  ...
  import * as EM316CONSTS from '@/consts/EM316_CONSTS'
  ...
  export default {
    ...
    methods: {
      ...
      loadData() {
        ...
        if (this.id) {
          getByEntId(this.id, '316').then((response) => {
        ...
        this.$refs.form.validate((formValid) => {
          if (formValid) {
            ...
            if (this.id) {
              ...
              updateEntrust(this.id, this.form, '316').then(
                (res) => {
                  getBySampleKey(this.form.sampnum, '316').then((response) => {
                    this.form = response.data
      ...
      async loadHisColunmDatas() {
        this.listLoading = true
        const { data } = await hislist(EM316CONSTS.ELMT_TYPE, EM316CONSTS.ELMT_CODE_STRUCTPARTS)
        this.structparts = data //结构部位
        await hislist(EM316CONSTS.ELMT_TYPE, EM316CONSTS.ELMT_CODE_CHECKTYPES).then((response) => {
          this.checktypes = response.data //检验类别
        })
        await hislist(EM316CONSTS.ELMT_TYPE, EM316CONSTS.ELMT_CODE_PIHAOS).then((response) => {
          this.pihaos = response.data //炉、批号
        })
        await hislist(EM316CONSTS.ELMT_TYPE, EM316CONSTS.ELMT_CODE_UNITNUMS).then((response) => {
          this.unitnums = response.data //生产厂家
        })
        await hislist(EM316CONSTS.ELMT_TYPE, EM316CONSTS.ELMT_CODE_STANDNAMES).then((response) => {
          this.standnames = response.data //检验标准
        })
        await hislist(EM316CONSTS.ELMT_TYPE, EM316CONSTS.ELMT_CODE_EXPLS).then((response) => {
          this.expls = response.data //备注
        })
        await hislist(EM316CONSTS.ELMT_TYPE, EM316CONSTS.ELMT_CODE_BKCOL1S).then((response) => {
          this.bkcol1s = response.data //母材单号
        })
        await hislist(EM316CONSTS.ELMT_TYPE, EM316CONSTS.ELMT_CODE_BKCOL2S).then((response) => {
          this.bkcol2s = response.data //技术等级
        })
        await hislist(EM316CONSTS.ELMT_TYPE, EM316CONSTS.ELMT_CODE_BKCOL3S).then((response) => {
          this.bkcol3s = response.data //焊条型号
        })
        await hislist(EM316CONSTS.ELMT_TYPE, EM316CONSTS.ELMT_CODE_RECADDS).then((response) => {
          this.recadds = response.data //取样地点
        })
        await hislist(EM316CONSTS.ELMT_TYPE, EM316CONSTS.ELMT_CODE_PAIHAOS).then((response) => {
          this.paihaos = response.data //牌号
        })
        await hislist(EM316CONSTS.ELMT_TYPE, EM316CONSTS.ELMT_CODE_SAMPNUMS).then((response) => {
