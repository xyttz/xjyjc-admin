<template>
  <div v-loading="listLoading" class="app-container">
    <table style="width: 100%; border-collapse: collapse">
      <tr>
        <td colspan="8" style="background-color: #5e5c5f; color: #fff">基础信息-自粘聚合物改性沥青防水卷材</td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td colspan="6">
          <el-checkbox v-model="form.checkitem2">拉力</el-checkbox>
          <el-checkbox v-model="form.checkitem3">延伸率</el-checkbox>
          <el-checkbox v-model="form.checkitem4">耐热性</el-checkbox>
          <el-checkbox v-model="form.checkitem5">低温柔性</el-checkbox>
          <el-checkbox v-model="form.checkitem6">不透水性</el-checkbox>
        </td>
      </tr>
      <tr>
      ...
      <tr>
        <td colspan="8">
          <el-form ref="form" :inline="true" label-width="150px" :model="form" :rules="rules" size="small">
            ...
            <el-form-item label="胎基" prop="type">
            ...
            <el-form-item label="粘结表面" prop="csbm">
              <el-select v-model="form.sbmcl" placeholder="请选择上表面材料" style="width: 200px">
                <el-option v-for="item in cSbmOptions" :key="item.name" :label="item.name" :value="item.value" />
              </el-select>
            </el-form-item>
            <el-form-item label="型号" prop="sort">
              <el-select v-model="form.sort" placeholder="请选择型号" style="width: 200px">
                <el-option v-for="item in sortOptions" :key="item.name" :label="item.name" :value="item.value" />
              </el-select>
            </el-form-item>
            <el-form-item label="公称厚度" prop="gcbh">
              <el-input v-model="form.gcbh" style="width: 200px" />
            </el-form-item>
            <el-form-item label="商标" prop="csb">
              <el-input v-model="form.cSb" style="width: 200px" />
            </el-form-item>
            ...
            <el-form-item label="批号" prop="cph">
              <el-input v-model="form.cPh" style="width: 200px" />
            </el-form-item>
            ...
            <el-form-item label="规格型号" prop="cSt">
              <el-input v-model="form.cSt" style="width: 200px" />
            </el-form-item>
...
<script>
  ...
  import * as EM323CONSTS from '@/consts/EM323_CONSTS'
  ...
  export default {
    ...
    methods: {
      ...
      loadData() {
        ...
        if (this.id) {
          getByEntId(this.id, '323').then((response) => {
        ...
        this.$refs.form.validate((formValid) => {
          if (formValid) {
            ...
            if (this.id) {
              ...
              updateEntrust(this.id, this.form, '323').then(
                (res) => {
                  getBySampleKey(this.form.sampnum, '323').then((response) => {
                    this.form = response.data
      ...
      async loadHisColunmDatas() {
        this.listLoading = true
        const { data } = await hislist(EM323CONSTS.ELMT_TYPE, EM323CONSTS.ELMT_CODE_STRUCTPARTS)
        this.structparts = data //结构部位
        await hislist(EM323CONSTS.ELMT_TYPE, EM323CONSTS.ELMT_CODE_CHECKTYPES).then((response) => {
          this.checktypes = response.data //检验类别
        })
        await hislist(EM323CONSTS.ELMT_TYPE, EM323CONSTS.ELMT_CODE_PIHAOS).then((response) => {
          this.pihaos = response.data //炉、批号
        })
        await hislist(EM323CONSTS.ELMT_TYPE, EM323CONSTS.ELMT_CODE_UNITNUMS).then((response) => {
          this.unitnums = response.data //生产厂家
        })
        await hislist(EM323CONSTS.ELMT_TYPE, EM323CONSTS.ELMT_CODE_STANDNAMES).then((response) => {
          this.standnames = response.data //检验标准
        })
        await hislist(EM323CONSTS.ELMT_TYPE, EM323CONSTS.ELMT_CODE_EXPLS).then((response) => {
          this.expls = response.data //备注
        })
        await hislist(EM323CONSTS.ELMT_TYPE, EM323CONSTS.ELMT_CODE_BKCOL1S).then((response) => {
          this.bkcol1s = response.data //母材单号
        })
        await hislist(EM323CONSTS.ELMT_TYPE, EM323CONSTS.ELMT_CODE_BKCOL2S).then((response) => {
          this.bkcol2s = response.data //技术等级
        })
        await hislist(EM323CONSTS.ELMT_TYPE, EM323CONSTS.ELMT_CODE_BKCOL3S).then((response) => {
          this.bkcol3s = response.data //焊条型号
        })
        await hislist(EM323CONSTS.ELMT_TYPE, EM323CONSTS.ELMT_CODE_RECADDS).then((response) => {
          this.recadds = response.data //取样地点
        })
        await hislist(EM323CONSTS.ELMT_TYPE, EM323CONSTS.ELMT_CODE_PAIHAOS).then((response) => {
          this.paihaos = response.data //牌号
        })
        await hislist(EM323CONSTS.ELMT_TYPE, EM323CONSTS.ELMT_CODE_SAMPNUMS).then((response) => {
