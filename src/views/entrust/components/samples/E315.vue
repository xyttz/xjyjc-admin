<template>
  <div v-loading="listLoading" class="app-container">
    <table style="width: 100%; border-collapse: collapse">
      <tr>
        <td colspan="8" style="background-color: #5e5c5f; color: #fff">基础信息-普通混凝土小型空心砌砖</td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td colspan="6">
          ...
          <el-checkbox v-model="form.checkitem2">密度</el-checkbox>
          <!-- <el-checkbox v-model="form.checkitem3">抗渗性</el-checkbox> -->
        </td>
      </tr>
      <tr>
      ...
      <tr>
        <td colspan="8">
          <el-form ref="form" :inline="true" label-width="150px" :model="form" :rules="rules" size="small">
            ...
            <el-form-item label="产地" prop="ccd">
              <el-input v-model="form.cCd" style="width: 200px" />
            </el-form-item>
            <el-form-item label="使用地区湿度" prop="csd">
              <el-select v-model="form.cSd" placeholder="请选择使用地区湿度" style="width: 200px">
                <el-option v-for="item in cSdOptions" :key="item.name" :label="item.name" :value="item.value" />
              </el-select>
            </el-form-item>
            <el-form-item label="龄期" prop="clq">
              <el-input v-model="form.cLq" style="width: 200px" />
            </el-form-item>
            ...
            <el-form-item label="当天龄期" prop="clq">
              <el-input v-model="form.clq" style="width: 200px" />
            </el-form-item>
      ...
      <tr>
        <td colspan="8">
          <el-form ref="attchForm" :inline="true" label-width="150px" :model="attchForm" size="small">
            ...
            <el-form-item label="抗冻标号" prop="ckdbh">
              <!-- <el-input v-model="form.recadd" placeholder="请输入取样地点" style="width: 200px" /> -->
              <el-autocomplete
                v-model="form.ckdbh"
                :fetch-suggestions="queryRecaddSearchAsync"
...
<script>
  ...
  import * as EM315CONSTS from '@/consts/EM315_CONSTS'
  ...
  export default {
    ...
    methods: {
      ...
      loadData() {
        ...
        if (this.id) {
          getByEntId(this.id, '315').then((response) => {
        ...
        this.$refs.form.validate((formValid) => {
          if (formValid) {
            ...
            if (this.id) {
              ...
              updateEntrust(this.id, this.form, '315').then(
                (res) => {
                  getBySampleKey(this.form.sampnum, '315').then((response) => {
                    this.form = response.data
      ...
      async loadHisColunmDatas() {
        this.listLoading = true
        const { data } = await hislist(EM315CONSTS.ELMT_TYPE, EM315CONSTS.ELMT_CODE_STRUCTPARTS)
        this.structparts = data //结构部位
        await hislist(EM315CONSTS.ELMT_TYPE, EM315CONSTS.ELMT_CODE_CHECKTYPES).then((response) => {
          this.checktypes = response.data //检验类别
        })
        await hislist(EM315CONSTS.ELMT_TYPE, EM315CONSTS.ELMT_CODE_PIHAOS).then((response) => {
          this.pihaos = response.data //炉、批号
        })
        await hislist(EM315CONSTS.ELMT_TYPE, EM315CONSTS.ELMT_CODE_UNITNUMS).then((response) => {
          this.unitnums = response.data //生产厂家
        })
        await hislist(EM315CONSTS.ELMT_TYPE, EM315CONSTS.ELMT_CODE_STANDNAMES).then((response) => {
          this.standnames = response.data //检验标准
        })
        await hislist(EM315CONSTS.ELMT_TYPE, EM315CONSTS.ELMT_CODE_EXPLS).then((response) => {
          this.expls = response.data //备注
        })
        await hislist(EM315CONSTS.ELMT_TYPE, EM315CONSTS.ELMT_CODE_BKCOL1S).then((response) => {
          this.bkcol1s = response.data //母材单号
        })
        await hislist(EM315CONSTS.ELMT_TYPE, EM315CONSTS.ELMT_CODE_BKCOL2S).then((response) => {
          this.bkcol2s = response.data //技术等级
        })
        await hislist(EM315CONSTS.ELMT_TYPE, EM315CONSTS.ELMT_CODE_BKCOL3S).then((response) => {
          this.bkcol3s = response.data //焊条型号
        })
        await hislist(EM315CONSTS.ELMT_TYPE, EM315CONSTS.ELMT_CODE_RECADDS).then((response) => {
          this.recadds = response.data //取样地点
        })
        await hislist(EM315CONSTS.ELMT_TYPE, EM315CONSTS.ELMT_CODE_PAIHAOS).then((response) => {
          this.paihaos = response.data //牌号
        })
        await hislist(EM315CONSTS.ELMT_TYPE, EM315CONSTS.ELMT_CODE_SAMPNUMS).then((response) => {
