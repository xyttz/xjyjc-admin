<template>
  <div v-loading="listLoading" class="app-container">
    <table style="width: 100%; border-collapse: collapse">
      <tr>
        <td colspan="8" style="background-color: #5e5c5f; color: #fff">基础信息-混凝土普通砖</td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td colspan="6">
          ...
          <el-checkbox v-model="form.checkitem2">密度试验</el-checkbox>
          <el-checkbox v-model="form.checkitem3">抗冻性</el-checkbox>
          <el-checkbox v-model="form.checkitem4">尺寸与外观</el-checkbox>
          <el-checkbox v-model="form.checkitem5">吸水率</el-checkbox>
          <el-checkbox v-model="form.checkitem6">颜色</el-checkbox>
        </td>
      </tr>
      <tr>
      ...
      <tr>
        <td colspan="8">
          <el-form ref="form" :inline="true" label-width="150px" :model="form" :rules="rules" size="small">
            ...
            <el-form-item label="强度等级" prop="cgrade">
            ...
            <el-form-item label="产品等级" prop="cdj">
              <el-select v-model="form.cDj" placeholder="请选择产品等级" style="width: 200px">
                <el-option v-for="item in cDjOptions" :key="item.name" :label="item.name" :value="item.value" />
              </el-select>
            </el-form-item>
            <el-form-item label="密度等级" prop="mddengji">
            ...
            <el-form-item label="砖的颜色" prop="ccolors">
              <el-input v-model="form.cColors" style="width: 200px" />
            </el-form-item>
            <el-form-item label="使用部位" prop="cBw">
              <el-select v-model="form.cBw" placeholder="请选择使用部位" style="width: 200px">
                <el-option v-for="item in cBwOptions" :key="item.name" :label="item.name" :value="item.value" />
              </el-select>
            </el-form-item>
...
<script>
  ...
  import * as EM313CONSTS from '@/consts/EM313_CONSTS'
  ...
  export default {
    ...
    methods: {
      ...
      loadData() {
        ...
        if (this.id) {
          getByEntId(this.id, '313').then((response) => {
        ...
        this.$refs.form.validate((formValid) => {
          if (formValid) {
            ...
            if (this.id) {
              ...
              updateEntrust(this.id, this.form, '313').then(
                (res) => {
                  getBySampleKey(this.form.sampnum, '313').then((response) => {
                    this.form = response.data
      ...
      async loadHisColunmDatas() {
        this.listLoading = true
        const { data } = await hislist(EM313CONSTS.ELMT_TYPE, EM313CONSTS.ELMT_CODE_STRUCTPARTS)
        this.structparts = data //结构部位
        await hislist(EM313CONSTS.ELMT_TYPE, EM313CONSTS.ELMT_CODE_CHECKTYPES).then((response) => {
          this.checktypes = response.data //检验类别
        })
        await hislist(EM313CONSTS.ELMT_TYPE, EM313CONSTS.ELMT_CODE_PIHAOS).then((response) => {
          this.pihaos = response.data //炉、批号
        })
        await hislist(EM313CONSTS.ELMT_TYPE, EM313CONSTS.ELMT_CODE_UNITNUMS).then((response) => {
          this.unitnums = response.data //生产厂家
        })
        await hislist(EM313CONSTS.ELMT_TYPE, EM313CONSTS.ELMT_CODE_STANDNAMES).then((response) => {
          this.standnames = response.data //检验标准
        })
        await hislist(EM313CONSTS.ELMT_TYPE, EM313CONSTS.ELMT_CODE_EXPLS).then((response) => {
          this.expls = response.data //备注
        })
        await hislist(EM313CONSTS.ELMT_TYPE, EM313CONSTS.ELMT_CODE_BKCOL1S).then((response) => {
          this.bkcol1s = response.data //母材单号
        })
        await hislist(EM313CONSTS.ELMT_TYPE, EM313CONSTS.ELMT_CODE_BKCOL2S).then((response) => {
          this.bkcol2s = response.data //技术等级
        })
        await hislist(EM313CONSTS.ELMT_TYPE, EM313CONSTS.ELMT_CODE_BKCOL3S).then((response) => {
          this.bkcol3s = response.data //焊条型号
        })
        await hislist(EM313CONSTS.ELMT_TYPE, EM313CONSTS.ELMT_CODE_RECADDS).then((response) => {
          this.recadds = response.data //取样地点
        })
        await hislist(EM313CONSTS.ELMT_TYPE, EM313CONSTS.ELMT_CODE_PAIHAOS).then((response) => {
          this.paihaos = response.data //牌号
        })
        await hislist(EM313CONSTS.ELMT_TYPE, EM313CONSTS.ELMT_CODE_SAMPNUMS).then((response) => {
