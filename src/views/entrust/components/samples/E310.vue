<template>
  <div v-loading="listLoading" class="app-container">
    <table style="width: 100%; border-collapse: collapse">
      <tr>
        <td colspan="8" style="background-color: #5e5c5f; color: #fff">基础信息-石子物理性能（国标）</td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td colspan="6">
          <el-checkbox v-model="form.checkitem1">含泥量</el-checkbox>
          <el-checkbox v-model="form.checkitem2">泥块含量</el-checkbox>
          <el-checkbox v-model="form.checkitem3">颗科级配</el-checkbox>
        </td>
      </tr>
      <tr>
      ...
      <tr>
        <td colspan="8">
          <el-form ref="form" :inline="true" label-width="150px" :model="form" :rules="rules" size="small">
            ...
            <el-form-item label="石品种" prop="szvar">
              <el-select v-model="form.szVar" placeholder="请选择石品种" style="width: 200px">
                <el-option v-for="item in szVarOptions" :key="item.name" :label="item.name" :value="item.value" />
              </el-select>
            </el-form-item>
            <el-form-item label="级配规格" prop="spec">
              <el-select v-model="form.spec" placeholder="请选择石品种" style="width: 200px">
                <el-option v-for="item in specOptions" :key="item.name" :label="item.name" :value="item.value" />
              </el-select>
            </el-form-item>
            ...
            <!-- <el-form-item label="级配类型" prop="silx">
              <el-select v-model="form.silx" placeholder="请选择级配类型" style="width: 200px">
                <el-option v-for="item in sampleStatusOptions" :key="item.name" :label="item.name" :value="item.name" />
              </el-select>
            </el-form-item> -->
            <el-form-item label="石类别" prop="szsort">
              <el-select v-model="form.szSort" placeholder="请选择石类别" style="width: 200px">
                <el-option v-for="item in szSortOptions" :key="item.name" :label="item.name" :value="item.value" />
              </el-select>
            </el-form-item>
            <el-form-item label="产地" prop="chprode">
              <el-input v-model="form.chProde" controls-position="right" :min="1" style="width: 200px" />
            </el-form-item>
      ...
      <tr>
        <td colspan="8">
          <el-form ref="attchForm" :inline="true" label-width="150px" :model="attchForm" size="small">
            ...
            <el-form-item label="供销单位" prop="bcfuc">
              <!-- <el-input v-model="form.recadd" placeholder="请输入取样地点" style="width: 200px" /> -->
              <el-autocomplete
                v-model="form.bcFuc"
                :fetch-suggestions="queryBcFucSearchAsync"
                placeholder="请选择供销单位"
                select-when-unmatched
                style="width: 200px"
                @select="handleSelect"
...
<script>
  ...
  import * as EM310CONSTS from '@/consts/EM310_CONSTS'
  ...
  export default {
    ...
    methods: {
      ...
      loadData() {
        ...
        if (this.id) {
          getByEntId(this.id, '310').then((response) => {
        ...
        this.$refs.form.validate((formValid) => {
          if (formValid) {
            ...
            if (this.id) {
              ...
              updateEntrust(this.id, this.form, '310').then(
                (res) => {
                  getBySampleKey(this.form.sampnum, '310').then((response) => {
                    this.form = response.data
      ...
      async loadHisColunmDatas() {
        this.listLoading = true
        const { data } = await hislist(EM310CONSTS.ELMT_TYPE, EM310CONSTS.ELMT_CODE_STRUCTPARTS)
        this.structparts = data //结构部位
        await hislist(EM310CONSTS.ELMT_TYPE, EM310CONSTS.ELMT_CODE_CHECKTYPES).then((response) => {
          this.checktypes = response.data //检验类别
        })
        await hislist(EM310CONSTS.ELMT_TYPE, EM310CONSTS.ELMT_CODE_PIHAOS).then((response) => {
          this.pihaos = response.data //炉、批号
        })
        await hislist(EM310CONSTS.ELMT_TYPE, EM310CONSTS.ELMT_CODE_UNITNUMS).then((response) => {
          this.unitnums = response.data //生产厂家
        })
        await hislist(EM310CONSTS.ELMT_TYPE, EM310CONSTS.ELMT_CODE_STANDNAMES).then((response) => {
          this.standnames = response.data //检验标准
        })
        await hislist(EM310CONSTS.ELMT_TYPE, EM310CONSTS.ELMT_CODE_EXPLS).then((response) => {
          this.expls = response.data //备注
        })
        await hislist(EM310CONSTS.ELMT_TYPE, EM310CONSTS.ELMT_CODE_BKCOL1S).then((response) => {
          this.bkcol1s = response.data //母材单号
        })
        await hislist(EM310CONSTS.ELMT_TYPE, EM310CONSTS.ELMT_CODE_BKCOL2S).then((response) => {
          this.bkcol2s = response.data //技术等级
        })
        await hislist(EM310CONSTS.ELMT_TYPE, EM310CONSTS.ELMT_CODE_BKCOL3S).then((response) => {
          this.bkcol3s = response.data //焊条型号
        })
        await hislist(EM310CONSTS.ELMT_TYPE, EM310CONSTS.ELMT_CODE_RECADDS).then((response) => {
          this.recadds = response.data //取样地点
        })
        await hislist(EM310CONSTS.ELMT_TYPE, EM310CONSTS.ELMT_CODE_PAIHAOS).then((response) => {
          this.paihaos = response.data //牌号
        })
        await hislist(EM310CONSTS.ELMT_TYPE, EM310CONSTS.ELMT_CODE_SAMPNUMS).then((response) => {
