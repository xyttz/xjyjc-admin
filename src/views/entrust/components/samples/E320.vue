<template>
  <div v-loading="listLoading" class="app-container">
    <table style="width: 100%; border-collapse: collapse">
      <tr>
        <td colspan="8" style="background-color: #5e5c5f; color: #fff">基础信息-砂浆试块抗压试验</td>
      </tr>
      <tr>
      ...
      <tr>
        <td colspan="8">
          <el-form ref="form" :inline="true" label-width="150px" :model="form" :rules="rules" size="small">
            ...
            <el-form-item label="砂浆品种" prop="cpz">
            ...
            <el-form-item label="养护条件" prop="cYh">
              <el-select v-model="form.cYh" placeholder="请选择砖品种" style="width: 200px">
                <el-option v-for="item in cYhOptions" :key="item.name" :label="item.name" :value="item.value" />
              </el-select>
            </el-form-item>
            <el-form-item label="尺寸" prop="size">
              <el-input v-model="form.size" style="width: 200px" />
            </el-form-item>
            <el-form-item label="成型日期" prop="repcondate">
              <el-date-picker
                v-model="form.cCxd"
                :editable="false"
                :picker-options="pickerOptions"
            ...
            <el-form-item label="当天龄期" prop="yqlq">
              <el-input v-model="form.yqlq" style="width: 200px" />
            </el-form-item>
            <el-form-item label="成型方法" prop="ccxf">
              <el-select v-model="form.cCxf" placeholder="请选择成型方法" style="width: 200px">
                <el-option v-for="item in cCxfOptions" :key="item.name" :label="item.name" :value="item.value" />
              </el-select>
            </el-form-item>
            <el-form-item label="配合比编号" prop="cphb">
              <el-input v-model="form.cPhb" style="width: 200px" />
            </el-form-item>
            <el-form-item label="要求龄期" prop="cqlq">
              <el-input v-model="form.cQlq" style="width: 200px" />
            </el-form-item>
      ...
      <tr>
        <td colspan="8">
          <el-form ref="attchForm" :inline="true" label-width="150px" :model="attchForm" size="small">
            <el-form-item label="制作人" prop="recadd">
...
<script>
  ...
  import * as EM320CONSTS from '@/consts/EM320_CONSTS'
  ...
  export default {
    ...
    methods: {
      ...
      loadData() {
        ...
        if (this.id) {
          getByEntId(this.id, '320').then((response) => {
        ...
        this.$refs.form.validate((formValid) => {
          if (formValid) {
            ...
            if (this.id) {
              ...
              updateEntrust(this.id, this.form, '320').then(
                (res) => {
                  getBySampleKey(this.form.sampnum, '320').then((response) => {
                    this.form = response.data
      ...
      async loadHisColunmDatas() {
        this.listLoading = true
        const { data } = await hislist(EM320CONSTS.ELMT_TYPE, EM320CONSTS.ELMT_CODE_STRUCTPARTS)
        this.structparts = data //结构部位
        await hislist(EM320CONSTS.ELMT_TYPE, EM320CONSTS.ELMT_CODE_CHECKTYPES).then((response) => {
          this.checktypes = response.data //检验类别
        })
        await hislist(EM320CONSTS.ELMT_TYPE, EM320CONSTS.ELMT_CODE_PIHAOS).then((response) => {
          this.pihaos = response.data //炉、批号
        })
        await hislist(EM320CONSTS.ELMT_TYPE, EM320CONSTS.ELMT_CODE_UNITNUMS).then((response) => {
          this.unitnums = response.data //生产厂家
        })
        await hislist(EM320CONSTS.ELMT_TYPE, EM320CONSTS.ELMT_CODE_STANDNAMES).then((response) => {
          this.standnames = response.data //检验标准
        })
        await hislist(EM320CONSTS.ELMT_TYPE, EM320CONSTS.ELMT_CODE_EXPLS).then((response) => {
          this.expls = response.data //备注
        })
        await hislist(EM320CONSTS.ELMT_TYPE, EM320CONSTS.ELMT_CODE_BKCOL1S).then((response) => {
          this.bkcol1s = response.data //母材单号
        })
        await hislist(EM320CONSTS.ELMT_TYPE, EM320CONSTS.ELMT_CODE_BKCOL2S).then((response) => {
          this.bkcol2s = response.data //技术等级
        })
        await hislist(EM320CONSTS.ELMT_TYPE, EM320CONSTS.ELMT_CODE_BKCOL3S).then((response) => {
          this.bkcol3s = response.data //焊条型号
        })
        await hislist(EM320CONSTS.ELMT_TYPE, EM320CONSTS.ELMT_CODE_RECADDS).then((response) => {
          this.recadds = response.data //取样地点
        })
        await hislist(EM320CONSTS.ELMT_TYPE, EM320CONSTS.ELMT_CODE_PAIHAOS).then((response) => {
          this.paihaos = response.data //牌号
        })
        await hislist(EM320CONSTS.ELMT_TYPE, EM320CONSTS.ELMT_CODE_SAMPNUMS).then((response) => {
