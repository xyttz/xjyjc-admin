<template>
  <div v-loading="listLoading" class="app-container">
    <table style="width: 100%; border-collapse: collapse">
      <tr>
        <td colspan="8" style="background-color: #5e5c5f; color: #fff">基础信息-土壤击实</td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td colspan="6">
          <el-checkbox v-model="form.checkitem1">土壤击实</el-checkbox>
        </td>
      </tr>
      <tr>
      ...
      <tr>
        <td colspan="8">
          <el-form ref="form" :inline="true" label-width="150px" :model="form" :rules="rules" size="small">
            ...
            <el-form-item label="土壤类别" prop="sampname">
            ...
            <el-form-item label="击实类型" prop="cjslx">
              <el-select v-model="form.cJslx" placeholder="请选择样品状态" style="width: 200px">
                <el-option v-for="item in cJslxOptions" :key="item.name" :label="item.name" :value="item.value" />
              </el-select>
            </el-form-item>
            ...
            <el-form-item label="最大粒径(mm)" prop="cblj">
              <el-input v-model="form.cBlj" style="width: 200px" />
            </el-form-item>
            ...
            <el-form-item label="取样位置" prop="cwz">
              <el-input v-model="form.cWz" style="width: 200px" />
            </el-form-item>
            <el-form-item label="压实系数" prop="cysxs">
              <el-input v-model="form.cYsxs" style="width: 200px" />
            </el-form-item>
            <el-form-item label="压实层数" prop="cjscs">
              <el-input v-model="form.cJscs" style="width: 200px" />
            </el-form-item>
            ...
            <el-form-item label="代表批量" prop="sampbat">
      ...
      <tr>
        <td colspan="8">
          <el-form ref="attchForm" :inline="true" label-width="150px" :model="attchForm" size="small">
            <el-form-item label="体积比" prop="ctjb">
            ...
            <el-form-item label="供土单位" prop="bcfuc">
...
<script>
  ...
  import * as EM325CONSTS from '@/consts/EM325_CONSTS'
  ...
  export default {
    ...
    methods: {
      ...
      loadData() {
        ...
        if (this.id) {
          getByEntId(this.id, '325').then((response) => {
        ...
        this.$refs.form.validate((formValid) => {
          if (formValid) {
            ...
            if (this.id) {
              ...
              updateEntrust(this.id, this.form, '325').then(
                (res) => {
                  getBySampleKey(this.form.sampnum, '325').then((response) => {
                    this.form = response.data
      ...
      async loadHisColunmDatas() {
        this.listLoading = true
        const { data } = await hislist(EM325CONSTS.ELMT_TYPE, EM325CONSTS.ELMT_CODE_STRUCTPARTS)
        this.structparts = data //结构部位
        await hislist(EM325CONSTS.ELMT_TYPE, EM325CONSTS.ELMT_CODE_CHECKTYPES).then((response) => {
          this.checktypes = response.data //检验类别
        })
        await hislist(EM325CONSTS.ELMT_TYPE, EM325CONSTS.ELMT_CODE_PIHAOS).then((response) => {
          this.pihaos = response.data //炉、批号
        })
        await hislist(EM325CONSTS.ELMT_TYPE, EM325CONSTS.ELMT_CODE_UNITNUMS).then((response) => {
          this.unitnums = response.data //生产厂家
        })
        await hislist(EM325CONSTS.ELMT_TYPE, EM325CONSTS.ELMT_CODE_STANDNAMES).then((response) => {
          this.standnames = response.data //检验标准
        })
        await hislist(EM325CONSTS.ELMT_TYPE, EM325CONSTS.ELMT_CODE_EXPLS).then((response) => {
          this.expls = response.data //备注
        })
        await hislist(EM325CONSTS.ELMT_TYPE, EM325CONSTS.ELMT_CODE_BKCOL1S).then((response) => {
          this.bkcol1s = response.data //母材单号
        })
        await hislist(EM325CONSTS.ELMT_TYPE, EM325CONSTS.ELMT_CODE_BKCOL2S).then((response) => {
          this.bkcol2s = response.data //技术等级
        })
        await hislist(EM325CONSTS.ELMT_TYPE, EM325CONSTS.ELMT_CODE_BKCOL3S).then((response) => {
          this.bkcol3s = response.data //焊条型号
        })
        await hislist(EM325CONSTS.ELMT_TYPE, EM325CONSTS.ELMT_CODE_RECADDS).then((response) => {
          this.recadds = response.data //取样地点
        })
        await hislist(EM325CONSTS.ELMT_TYPE, EM325CONSTS.ELMT_CODE_PAIHAOS).then((response) => {
          this.paihaos = response.data //牌号
        })
        await hislist(EM325CONSTS.ELMT_TYPE, EM325CONSTS.ELMT_CODE_SAMPNUMS).then((response) => {
