<template>
  <div v-loading="listLoading" class="app-container">
    <table style="width: 100%; border-collapse: collapse">
      <tr>
        <td colspan="8" style="background-color: #5e5c5f; color: #fff">基础信息-砂子物理性能（国标）</td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <td colspan="6">
          <el-checkbox v-model="form.checkitem1">颗粒级配</el-checkbox>
          <el-checkbox v-model="form.checkitem2">含泥量</el-checkbox>
          <el-checkbox v-model="form.checkitem4">泥块含量</el-checkbox>
        </td>
      </tr>
      <tr>
        ...
        <td colspan="8">
          <el-form ref="form" :inline="true" label-width="150px" :model="form" :rules="rules" size="small">
            ...
            <el-form-item label="规格" prop="spec">
              <el-select v-model="form.spec" placeholder="请选择品种" style="width: 200px">
            ...
            <el-form-item label="砂类别" prop="szsort">
...
<script>
  ...
  import * as EM309CONSTS from '@/consts/EM309_CONSTS'
  ...
  export default {
    ...
    methods: {
      ...
      loadData() {
        ...
        if (this.id) {
          getByEntId(this.id, '309').then((response) => {
        ...
        this.$refs.form.validate((formValid) => {
          if (formValid) {
            ...
            if (this.id) {
              ...
              updateEntrust(this.id, this.form, '309').then(
                (res) => {
                  getBySampleKey(this.form.sampnum, '309').then((response) => {
                    this.form = response.data
                    this.id = this.form.id
                    if ('1100000000000000000000000000000000000001' == this.form.sampphase) {
                      this.showQrCdoe = true
                      this.qrvalue = this.form.entkey + '##' + this.form.entnum + '##xz_ent_szig##xz_ent_szig'
                    } else {
                      this.showQrCdoe = false
                      this.qrvalue = 'Hello world'
                    }
      ...
      queryBcFucSearchAsync(queryString, cb) {
        const restaurants = this.recadds
        const results = queryString ? restaurants.filter(this.createStateFilterParent(queryString)) : restaurants
        clearTimeout(this.timeout)
        this.timeout = setTimeout(() => {
      ...
      async loadHisColunmDatas() {
        this.listLoading = true
        const { data } = await hislist(EM309CONSTS.ELMT_TYPE, EM309CONSTS.ELMT_CODE_STRUCTPARTS)
        this.structparts = data //结构部位
        await hislist(EM309CONSTS.ELMT_TYPE, EM309CONSTS.ELMT_CODE_CHECKTYPES).then((response) => {
          this.checktypes = response.data //检验类别
        })
        await hislist(EM309CONSTS.ELMT_TYPE, EM309CONSTS.ELMT_CODE_PIHAOS).then((response) => {
          this.pihaos = response.data //炉、批号
        })
        await hislist(EM309CONSTS.ELMT_TYPE, EM309CONSTS.ELMT_CODE_UNITNUMS).then((response) => {
          this.unitnums = response.data //生产厂家
        })
        await hislist(EM309CONSTS.ELMT_TYPE, EM309CONSTS.ELMT_CODE_STANDNAMES).then((response) => {
          this.standnames = response.data //检验标准
        })
        await hislist(EM309CONSTS.ELMT_TYPE, EM309CONSTS.ELMT_CODE_EXPLS).then((response) => {
          this.expls = response.data //备注
        })
        await hislist(EM309CONSTS.ELMT_TYPE, EM309CONSTS.ELMT_CODE_BKCOL1S).then((response) => {
          this.bkcol1s = response.data //母材单号
        })
        await hislist(EM309CONSTS.ELMT_TYPE, EM309CONSTS.ELMT_CODE_BKCOL2S).then((response) => {
          this.bkcol2s = response.data //技术等级
        })
        await hislist(EM309CONSTS.ELMT_TYPE, EM309CONSTS.ELMT_CODE_BKCOL3S).then((response) => {
          this.bkcol3s = response.data //焊条型号
        })
        await hislist(EM309CONSTS.ELMT_TYPE, EM309CONSTS.ELMT_CODE_RECADDS).then((response) => {
          this.recadds = response.data //取样地点
        })
        await hislist(EM309CONSTS.ELMT_TYPE, EM309CONSTS.ELMT_CODE_PAIHAOS).then((response) => {
          this.paihaos = response.data //牌号
        })
        await hislist(EM309CONSTS.ELMT_TYPE, EM309CONSTS.ELMT_CODE_SAMPNUMS).then((response) => {
