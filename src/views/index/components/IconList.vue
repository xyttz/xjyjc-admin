<template>
  <div>
    <el-col v-for="(item, index) in iconList" :key="index" :lg="3" :md="3" :sm="6" :xl="3" :xs="12">
      <el-card v-if="item.click && item.click === 'changeTheme'" class="icon-panel" shadow="hover" @click.native="changeTheme">
        <vab-icon :icon="item.icon" :style="{ color: item.color }" />
        <p>{{ item.title }}</p>
      </el-card>
      <el-card v-else-if="item.click && item.click === 'randomTheme'" class="icon-panel" shadow="hover" @click.native="randomTheme">
        <el-badge value="点我">
          <vab-icon :icon="item.icon" :style="{ color: item.color }" />
        </el-badge>
        <p>{{ item.title }}</p>
      </el-card>
      <el-card v-else-if="item.click && item.click === 'handleMore'" class="icon-panel" shadow="hover" @click.native="handleMore">
        <vab-icon :icon="item.icon" :style="{ color: item.color }" />
        <p>{{ item.title }}</p>
      </el-card>
