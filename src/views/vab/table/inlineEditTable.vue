<template>
  <div class="inline-edit-table-container">
    <el-alert :closable="false" show-icon title="三级路由【不缓存路由并且不固定表格高度】的示例" type="success" />
    <vab-query-form>
    ...
    <el-table v-loading="listLoading" border :data="list">
      ...
      <el-table-column align="center" label="序号" show-overflow-tooltip width="95">
        <template #default="{ $index }">
          {{ $index + 1 }}
        </template>
      </el-table-column>
      <el-table-column align="center" label="标题" min-width="300px" show-overflow-tooltip>
      ...
      <el-table-column align="center" label="操作" show-overflow-tooltip width="200">
        <template #default="{ row }">
          <el-button v-if="row.edit" size="small" type="success" @click="confirmEdit(row)">保存</el-button>
          <el-button v-else icon="el-icon-edit" size="small" type="primary" @click="row.edit = !row.edit">编辑</el-button>
        </template>
      </el-table-column>
      <template #empty>
