  ...
  <div class="app-container">
    ...
    <el-form ref="queryForm" :inline="true" :model="queryParams" size="small">
      <el-form-item>
        ...
        <el-button :disabled="single" icon="el-icon-edit" type="success" @click="handleUpdate">修改</el-button>
        <el-button :disabled="multiple" icon="el-icon-delete" type="danger" @click="handleDelete">删除</el-button>
      </el-form-item>
      <el-form-item prop="clientId">
        <el-input v-model="queryParams.clientId" clearable placeholder="客户端ID" style="width: 240px" @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item>
    ...
    <!-- 数据表格 -->
    <el-table v-loading="loading" border :data="pageList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="域" prop="scope" width="100" />
      <el-table-column :formatter="autoapproveFormat" label="自动放行" prop="autoapprove" width="100" />
      <el-table-column :formatter="authorizedGrantTypesFormat" label="授权方式" prop="authorizedGrantTypes" />
      <el-table-column label="认证令牌时效(单位：秒)" prop="accessTokenValidity" width="200" />
      <el-table-column label="刷新令牌时效(单位：秒)" prop="refreshTokenValidity" width="200" />
      ...
      <el-form ref="form" label-width="100px" :model="form" :rules="rules">
        ...
        <el-form-item label="授权方式" prop="authorizedGrantTypes">
          <el-checkbox-group v-model="form.authorizedGrantTypes">
            <el-checkbox v-for="item in authorizedGrantTypesOptions" :key="item.value" :label="item.value">{{ item.name }}</el-checkbox>
          </el-checkbox-group>
        </el-form-item>
        ...
        <el-form-item label="扩展信息" prop="additionalInformation">
          <el-input v-model="form.additionalInformation" placeholder="JSON格式" type="textarea" />
        </el-form-item>
      </el-form>
    ...
    data() {
      return {
        ...
        rules: {
          ...
          authorizedGrantTypes: [{ required: true, message: '请选择授权方式', trigger: 'blur' }],
        },
      }
    },
    async created() {
