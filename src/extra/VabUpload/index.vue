  ...
  <el-dialog :before-close="handleClose" :close-on-click-modal="false" :title="title" :visible.sync="dialogFormVisible" width="909px">
    ...
    <template #footer>
      <div style="position: relative; padding-right: 15px; text-align: right">
        <div v-if="show" style="position: absolute; top: 10px; left: 15px; color: #999">正在上传中... 当前上传成功数:{{ imgSuccessNum }}张 当前上传失败数:{{ imgErrorNum }}张</div>
    ...
    methods: {
      ...
      handleSuccess(response, file, fileList) {
        ...
        if (fileList.length === this.imgNum) {
          setTimeout(() => {
            this.$baseMessage(`上传完成! 共上传${fileList.length}张图片`, 'success', 'vab-hey-message-success')
          }, 1000)
        }
      ...
      handleError(err, file) {
        ...
        this.$baseMessage(`文件[${file.raw.name}]上传失败,文件大小为${_.round(file.raw.size / 1024, 0)}KB`, 'error', 'vab-hey-message-error')
        setTimeout(() => {
          this.loading = false
          this.show = false
        }, 1000)
