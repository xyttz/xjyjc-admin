...
const responseFake = (url, type, respond) => {
  return {
    ...
    response(req, res) {
      ...
      if (JSON.stringify(req.body) !== '{}') console.log(chalkNext.green(`> 请求参数(body)：${JSON.stringify(req.body)}`))
      if (JSON.stringify(req.query) !== '{}') console.log(chalkNext.green(`> 请求参数(query)：${JSON.stringify(req.query)}`))
      ...
      if (event === 'change' || event === 'add') {
        try {
          ...
          Object.keys(require.cache).forEach((item) => {
            if (item.includes(mockDir)) delete require.cache[require.resolve(item)]
          })
          const mockRoutes = registerRoutes(app)
